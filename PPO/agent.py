...
class PPO:
    def __init__(self, state_dim, action_dim, gamma=0.99, continuous=False, policy_clip=0.2, n_epochs=1,
                 gae_lambda=0.95, device=DEVICE, hidden_dim=32, batch_size=1, actor_lr=0.0003, critic_lr=0.0003):
        self.gamma = gamma
    ...
    def choose_action(self, state):
        state = torch.unsqueeze(state, dim=0).clone().detach().to(self.device)
